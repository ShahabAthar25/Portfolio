  ...
  return (
    <Link href={`/projects/${project.slug}`}>
      <div
        tabIndex={0}
        role="button"
        className="group relative overflow-hidden border-b border-border cursor-pointer"
      >
        ...
        <div className="relative z-10 max-w-6xl mx-auto px-6 py-10 flex items-start justify-between gap-6">
          <div className="flex-1 min-w-0">
            ...
            <div className="max-h-0 opacity-0 group-hover:max-h-[60px] group-hover:opacity-100 transition-all duration-350 ease-out pl-[calc(1.5rem+1px)] overflow-hidden">
            ...
            <div className="flex flex-wrap items-center gap-x-3 gap-y-1.5 mt-4 pl-[calc(1.5rem+1px)]">
              ...
              {project.tech.slice(0, 3).map((t) => (
                <span
                  key={
                    t
                  } /* 5. Replaced inline border-color style with group-hover:border */
          ...
          <div className="flex flex-col items-end gap-2 pt-2 flex-shrink-0">
            <ArrowUpRight className="w-7 h-7 text-muted-foreground group-hover:text-[var(--accent)] group-hover:translate-x-[3px] group-hover:-translate-y-[3px] transition-all duration-300 ease-out" />

            <span className="text-xs text-[var(--accent)] opacity-0 translate-y-1 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-250 ease-out whitespace-nowrap">
              Case Study →
            </span>
          </div>
        </div>
