  ...
  <div className="relative z-10 max-w-6xl mx-auto px-6 py-10 flex items-start justify-between gap-6">
    <div className="flex-1 min-w-0">
      <div className="flex items-baseline gap-5 mb-1">
        <span className="text-xs text-foreground flex-shrink-0 tabular-nums font-mono">
          {project.id}
        </span>
        <h3
      ...
      <div
        ...
        <p className="text-sm text-foreground leading-relaxed pt-2 pb-1">
          {project.desc}
        </p>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-x-3 gap-y-1.5 mt-4 pl-[calc(1.5rem+1px)]">
        ...
        <span key={ t } /* 5. Replaced inline border-color style with group-hover:border */
          className="text-xs text-foreground border border-border group-hover:border-[rgba(197,241,53,0.35)] transition-colors duration-350 px-2 py-0.5">
          {t}
        </span>
        ))}
      </div>
    ...
    <div className="flex flex-col items-end gap-2 pt-2 flex-shrink-0">
      <ArrowUpRight
        className="w-7 h-7 text-foreground group-hover:text-[var(--accent)] group-hover:translate-x-[3px] group-hover:-translate-y-[3px] transition-all duration-300 ease-out" />
